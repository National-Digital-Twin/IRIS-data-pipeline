...
def add_typed_field(record: dict, type: str, type_ns: str, record_field: str) -> str:
    ...
    ies.add_triple(
        subject=identifier_uri,
        predicate=build_ies_uri("representationValue"),
        obj=value,
        is_literal=True,
        literal_type="string",
    )
...
def add_typed_identifier(
    record: dict, subject: str, type: str, type_ns: str, record_field: str
) -> None:
    """
    ...
    ies.add_triple(
        subject, predicate=build_ies_uri("isIdentifiedBy"), obj=identifier_uri
    )
...
def add_postcode_identifier(record: dict, addressable_location: str) -> None:
    ...
    ies.add_triple(
        addressable_location, predicate=ies_ns + "isIdentifiedBy", obj=identifier_uri
    )


def add_addressable_location_identifiers(
    record: dict, addressable_location: str
) -> None:
    """
    ...
    add_typed_identifier(
        record, addressable_location, "FirstLineOfAddress", ies_ns, "fulladdress"
    )
...
def add_bnode_with_ies_type_and_value(type: str, literal_value: str) -> BNode:
    ...
    ies.graph.add(
        (
            ...
            Literal(literal_value, datatype=XSD.string),
        )
    )
    return bnode
...
def add_geographic_mapping(record: dict) -> None:
    ...
    ies.add_triple(
        subject=location_point_uri,
        predicate=RDF_TYPE,
        obj=build_ies_uri("PointOnEarthSurface"),
    )

    long = record.get("longitude")
    long_node = add_bnode_with_ies_type_and_value("Longitude", long)
    ies.graph.add(
        (URIRef(location_point_uri), URIRef(build_ies_uri("isIdentifiedBy")), long_node)
    )
    lat = record.get("latitude")
    lat_node = add_bnode_with_ies_type_and_value("Latitude", lat)
    ies.graph.add(
        (URIRef(location_point_uri), URIRef(build_ies_uri("isIdentifiedBy")), lat_node)
    )
    ...
    ies.graph.add(
        (wkt_bnode, URIRef(build_ies_uri("representationValue")), wkt_literal)
    )
    ies.graph.add((wkt_bnode, GEO.asWKT, wkt_literal))
    ies.graph.add(
        (
            URIRef(location_point_uri),
            URIRef(build_ies_uri("isRepresentedAs")),
            wkt_bnode,
        )
    )
...
def map_func(record: dict) -> str:
    ...
    ies.add_triple(
        addressable_location_uri,
        RDF_TYPE,
        build_ies_building_uri("AddressableLocation"),
    )
