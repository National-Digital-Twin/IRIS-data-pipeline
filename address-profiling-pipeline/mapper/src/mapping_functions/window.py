...
def window_function(record: dict, ies, debug_mode=False):
    """
    Creates a graph serialized to n-triples containing details about the windows of a property.

    Args:
        record (dict): A record representing a building.

    Returns:
        str: The RDF graph serialized into n-triples containing details about the windows of a property.
    """
    ...
    if record["CertificateType"] == "domestic":
        ...
        if debug_mode:
            ies.graph.serialize(destination=f"{uprn}_window.ttl", format="turtle")
            return
